...
脚本同步执行端与两个应用的独立 Python 3.12 环境，首次询问控制台 HTTPS 地址、机器人连接凭据、聚水潭和京麦的用户名、密码、预期可见身份。控制台先创建机器人取得连接凭据；两个应用的账号别名均为 `STORE_001`。

路径与 WSS 地址自动填写到忽略的 `config.local.toml`。相对路径以配置文件目录为基准。ngrok 使用公开签发证书，不需要 CA 文件；私有 CA 仍可在 TOML 中配置 `ca_file`。

业务凭据通过 Windows DPAPI 加密保存在忽略的 `credentials.local.clixml`。该文件仅由同一电脑的同一 Windows 用户解密，不能复制到另一台机器直接使用。凭据在启动时注入执行端进程，Agent 不接触明文。
...
日期、品牌、文件名和下载目录在控制台创建 Run 时填写。活跃 Run 先在控制台请求停止并等到结束确认，再关闭执行端窗口；强行退出可能保留“状态待确认”。不要删除 `executor.sqlite` 或浏览器现场来强行解除占用。

Mac 启动步骤与真实验收状态见 [接管实施说明](../../../kk-rpa-dashboard/docs/recovery-implementation.md)。
