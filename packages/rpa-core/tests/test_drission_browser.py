...
def test_adapter_exclusive_custom_select_removes_extra_checked_option(
    ...
    frame = ExclusiveFrame({"#custom-brand": custom, "#dismiss-anchor": dismiss})
    ...
    assert events[-2:] == ["dismiss-wait", "dismiss-click"]
...
def test_adapter_relocates_frame_before_each_element_action(tmp_path: Path) -> None:
    ...
    browser.input(FRAMED_INPUT, "FIXTURE_BRAND")
...
def test_adapter_download_uses_the_configured_directory(
    tmp_path: Path, custom_directory: bool
) -> None:
    element = FakeElement()
    frame = FakeTab({FRAMED_DOWNLOAD.require_locator().value: element})
    tab = FakeTab(frames={"#product-stock-frame": frame})
    run_dir = tmp_path / "runs" / "one"
    configured = tmp_path / "shared-downloads" if custom_directory else None
