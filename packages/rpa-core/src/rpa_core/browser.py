...
DrissionPage adapter lives behind this contract; no DrissionPage type is
allowed to escape into application code.
"""
...
_ELEMENT_ID_PATTERN = re.compile("^[A-Za-z][A-Za-z0-9_.:-]*$")
...
class Locator:
    ...
    ``value`` is intentionally opaque to applications.  For the
...
class ElementSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if self.selected_option_locator is not None and self.option_locator is None:
            raise ValueError("selected_option_locator requires option_locator")
    ...
    def require_locator(self) -> Locator:
        if self.locator is None:
            raise UnresolvedElementError(f"element {self.id!r} has no verified locator")
        return self.locator
...
class FakeBrowserActions:
    ...
    run_dir: Path | str
    download_dir: Path | None = field(default=None, kw_only=True)
    ...
    def __post_init__(self) -> None:
        ...
        if invalid_list_ids:
            raise ValueError(
                f"invalid fake text-list element IDs: {invalid_list_ids!r}"
            )
    ...
    def count(self, element: ElementSpec, *, timeout: float = 0.0) -> int:
        ...
        if timeout < 0:
            raise ValueError("timeout must be non-negative")
        value = self._counts.get(element.id, 1 if element.id in self._visible else 0)
        self.actions.append(BrowserActionRecord("count", element.id, str(value)))
        return value
    ...
    def download(
        ...
        target_dir = self.download_dir or self.run_dir / "downloads"
    ...
    def screenshot(
        ...
        self.actions.append(
            BrowserActionRecord(
                "screenshot", detail="full_page" if full_page else "viewport"
            )
        )
        return reference
