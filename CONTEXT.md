...
**RPA Task**:
选择一个 RPA 应用、保存一套参数规则并指定机器人电脑的可重复执行配置。一个应用可以创建多个任务。
_Avoid_: 单次运行、业务 Step

**Console Run**:
一次手动、定时或 Agent 触发形成的执行请求，具有固定的账号和实际业务参数。一个任务可以产生多次控制台运行。
_Avoid_: 任务配置、执行尝试

**Execution Attempt**:
程序从头执行或从指定步骤续跑的一次过程。同一次控制台运行可以因 Agent 接管产生多次执行尝试。
_Avoid_: 新任务、从头重跑产生的新控制台运行

**Robot Computer**:
执行 RPA 应用的专用电脑，同一时刻由一次控制台运行占用。Agent 接管期间继续保持该次运行的占用。
_Avoid_: 调度 Agent、控制台服务器

**Application Import**:
将已有 RPA 应用及其参数声明纳入控制台可选应用的过程。导入完成表示应用已被识别，实际执行还取决于目标机器人的部署状态。
_Avoid_: 已部署、已通过真实运行验收
...
**Run Inputs**:
一次运行固定的实际业务参数，例如报表日期与品牌；续跑和从头重跑保持原值。
