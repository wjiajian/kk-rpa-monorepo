...
- check per-invocation parameters, preparation failures with run IDs, agent-supplied output validation and temporary locator isolation;
...
Fix failures caused by the current change, then repeat the affected checks. Report framework/offline application results separately from real element verification, real runs and business acceptance. State the actual reason for incomplete real validation, such as missing authorization, an unavailable environment or a specific failure; retain existing authorization. Do not claim historical runs validate new code.
